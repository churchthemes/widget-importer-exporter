...
function wie_add_import_export_page() {
	...
	// Page title.
		...
		// Menu title.
		esc_html__( 'Widget Importer & Exporter', 'widget-importer-exporter' ),
		// Capability (can manage Appearance > Widgets).
		'edit_theme_options',
		// Menu Slug.
...
function wie_import_export_page_content() {
	...
	<div class="wrap">
		...
		<p class="submit">
			<a href="<?php echo esc_url( admin_url( basename( $_SERVER['PHP_SELF'] ) . '?page=' . $_GET['page'] . '&export=1&wie_export_nonce=' . wp_create_nonce( 'wie_export' ) ) ); ?>"
			   id="wie-export-button" class="button button-primary">
...
function wie_footer() {
	...
	<div id="wie-support-project" class="wie-box">
		...
		<p>
			<a href="https://churchthemes.com/project-support/" class="button"
			   target="_blank"><?php esc_html_e( 'Give $5 or More', 'widget-importer-exporter' ); ?></a>
			<a href="https://wordpress.org/support/plugin/widget-importer-exporter/reviews/?filter=5" class="button"
			   target="_blank"><?php esc_html_e( 'Add Your Review', 'widget-importer-exporter' ); ?></a>
		</p>
